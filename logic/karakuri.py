...
# ====== 定数 ======
DEFAULT_COIN = 32.9     # コイン持ち（50枚あたりのG数）
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "karakuri.csv")
...
def _calc_cz(df: pd.DataFrame, payout_col: str,
    ...
    # ✅ 前回AT状況による有利フラグの絞り込み
    if prev_at_status == "下位AT後":
        base = base[base["有利フラグ"] == 0]
    elif prev_at_status == "運命の一撃失敗後":
        base = base[base["有利フラグ"].between(1, 2)]
    else:
        base = base[base["有利フラグ"].between(0, 2)]  # デフォルト保険
...
def karakuri_handler():
    ...
    if request.method == "POST":
        print("DEBUG - POSTデータ:", request.form.to_dict())  # ←追加
        ...
        if filters["target"] == "CZ狙い":
            ...
            result = _calc_hikimodoshi_like(df=df, g_threshold=95,
                                            start_g=filters["start"], through_input=filters["through"],
                                            diff_min=filters["差枚_min"], diff_max=filters["差枚_max"],
                                            remaining_minutes=filters["remaining"],
                                            prev_at_status=filters["prev_at_status"],
            ...
            result = {"機械割(%)": "-", "サンプル数": 0,
                      "平均獲得枚数": "-", "note": "対象外の狙いです"}

        show_result = True


    return render_template("karakuri.html", filters=filters,
