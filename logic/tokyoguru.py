...
# ====== 定数（デフォルト値） ======
DEFAULT_COIN = 31     # コイン持ち（50枚あたりのG数）
DEFAULT_JUNZOU = 4.0  # AT純増
...
CSV_PATH = os.path.join(os.path.dirname(__file__), "..", "data", "tokyoguru.csv")
...
def _calc_cz(df: pd.DataFrame, payout_col: str,
             *, start_g: int, through_input: int,
             ...
             COIN_MOCHI: float, JUNZOU: float, KAI: int) -> dict:
...
def tokyoguru_handler():
    filters = {
        ...
        "差枚_min": -5000,
        "差枚_max": 2500,
        "coin_mochi": 31,   # ✅ デフォルト 31
        "junzou": 4.0,      # ✅ デフォルト 4.0
        "kai": 800          # ✅ デフォルト 800
    }
    show_result = False
    result = None
    ...
    if request.method == "POST":
        ...
        if filters["target"] == "CZ狙い":
            ...
            result = _calc_hikimodoshi_like(df=df, g_threshold=40,
                                            start_g=filters["start"], through_input=filters["through"],
                                            diff_min=filters["差枚_min"], diff_max=filters["差枚_max"],
                                            remaining_minutes=filters["remaining"],
                                            COIN_MOCHI=COIN_MOCHI, JUNZOU=JUNZOU, KAI=KAI)
    ...
    return render_template("tokyoguru.html", filters=filters,
